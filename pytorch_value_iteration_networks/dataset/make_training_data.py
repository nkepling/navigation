...
def make_data(dom_size, n_domains, max_obs, max_obs_size, n_traj,
    ...
    print("n domains",n_domains)
    print("done size",dom_size)
    ...
    while dom <= n_domains:
        ...
        for i in range(n_traj):
            if len(states_xy[i]) > 1:
                ...
                X_current = np.tile(iv_mixed, (ns, 1, 1, 1)) # obstacles plus the value prior --- the location of the goal 
                # Resize states
                S1_current = np.expand_dims(states_xy[i][0:ns, 0], axis=1) # grpah all x coords 

                S2_current = np.expand_dims(states_xy[i][0:ns, 1], axis=1) # grab all y coords
                # Resize labels
    ...
    print("x input shape", X_f.shape)
    S1_f = np.concatenate(S1_l)
    print("agent x coord shape", S1_f.shape)
    S2_f = np.concatenate(S2_l)
    Labels_f = np.concatenate(Labels_l)
    print("action/tajectory labels shape",Labels_f.shape)
...
if __name__ == '__main__':
    ...
    parser.add_argument("--n_domains", "-nd", type=int, help="number of domains", default=1)
    ...
    parser.add_argument("--n_traj", "-nt", type=int, help="number of trajectories", default=1)
